...
// Minimal Joyride hook - ONLY reads DOM positions (unavoidable) and scrolls
// All logic is in Elixir - this just bridges the DOM gap
const Joyride = {
  mounted() {
    this.currentTarget = null;
    this.scrollHandler = () => this.reportPosition();
    this.resizeHandler = () => this.reportPosition();

    this.handleEvent("joyride:goto", ({ target, scroll, offset }) => {
      console.log("[Joyride] goto event received:", target, scroll, offset);
      this.currentTarget = target;
      this.gotoTarget(target, scroll, offset);
    });
  ...
  gotoTarget(targetId, shouldScroll, scrollOffset) {
    console.log("[Joyride] gotoTarget:", targetId);
    if (!targetId) return;

    const el = document.querySelector(`[data-joyride="${targetId}"]`);
    console.log("[Joyride] found element:", el);
    if (!el) {
      console.log("[Joyride] element NOT FOUND, pushing not_found event");
      this.pushEvent("not_found", { target: targetId });
      return;
    }
    ...
    if (shouldScroll) {
      ...
      if (!isInViewport) {
        ...
        // Wait for scroll to finish before reporting position
        setTimeout(() => this.reportPosition(), 400);
        return;
      }
    }
  ...
  reportPosition() {
    ...
    console.log("[Joyride] reportPosition for:", targetId);
    if (!targetId) return;

    const el = document.querySelector(`[data-joyride="${targetId}"]`);
    if (!el) {
      console.log("[Joyride] reportPosition: element not found");
      return;
    }

    const rect = el.getBoundingClientRect();
    console.log("[Joyride] pushing rect:", rect);
    this.pushEvent("rect", {
      rect: {
        x: rect.x,
        y: rect.y,
