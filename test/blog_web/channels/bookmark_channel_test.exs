defmodule BlogWeb.BookmarkChannelTest do
  ...
  describe "join bookmark:client:<user_id>" do
    test "joins successfully and returns user's bookmarks" do
      ...
      {:ok, bookmark1} = Store.add_bookmark("https://example.com", "Example", "Test site", [], nil, user_id)
      {:ok, bookmark2} = Store.add_bookmark("https://test.com", "Test", "Another test", ["test"], nil, user_id)
      
      # Join the channel
      {:ok, reply, socket} = join(socket(BlogWeb.UserSocket, "user_id", %{user_id: user_id}), BookmarkChannel, "bookmark:client:#{user_id}")
    ...
    test "different users get their own bookmarks" do
      ...
      # Join as user1
      {:ok, reply1, _} = join(socket(BlogWeb.UserSocket, "user_id", %{user_id: user1}), BookmarkChannel, "bookmark:client:#{user1}")
      
      # Join as user2
      {:ok, reply2, _} = join(socket(BlogWeb.UserSocket, "user_id", %{user_id: user2}), BookmarkChannel, "bookmark:client:#{user2}")
  ...
  describe "handle_in add_bookmark" do
    setup do
      user_id = "test_user"
      {:ok, _reply, socket} = join(socket(BlogWeb.UserSocket, "user_id", %{user_id: user_id}), BookmarkChannel, "bookmark:client:#{user_id}")
  ...
  describe "handle_in delete_bookmark" do
    setup do
      ...
      # Add a test bookmark
      {:ok, bookmark} = Store.add_bookmark("https://delete-me.com", "Delete Me", "", [], nil, user_id)
    ...
    test "handles deletion of nonexistent bookmark", %{socket: socket} do
      ...
      assert_reply ref, :ok  # Delete operations are idempotent
    end
  end
  ...
  describe "handle_in search_bookmarks" do
    setup do
      ...
      # Add test bookmarks
      {:ok, _} = Store.add_bookmark("https://elixir-lang.org", "Elixir Language", "Programming language", ["elixir", "programming"], nil, user_id)
      {:ok, _} = Store.add_bookmark("https://phoenixframework.org", "Phoenix Framework", "Web framework", ["phoenix", "web"], nil, user_id)
      {:ok, _} = Store.add_bookmark("https://github.com", "GitHub", "Code repository", ["git", "code"], nil, user_id)
  ...
  describe "handle_in bookmark:create (Chrome extension format)" do
    ...
    test "creates bookmark with minimal data", %{socket: socket} do
      ...
      assert bookmark.title == "https://minimal.com"  # Default to URL
