defmodule Blog.Integration.BookmarkFlowTest do
  ...
  describe "Complete bookmark workflow via channels" do
    ...
    test "firehose broadcasts work across multiple users", %{user_id: user_id} do
      ...
      {:ok, _reply, socket2} =
        Phoenix.ChannelTest.join(
          socket(BlogWeb.UserSocket, "user_id", %{user_id: user_id_2}),
          BookmarkChannel,
          "bookmark:client:#{user_id_2}"
        )
      ...
      ref =
        Phoenix.ChannelTest.push(socket2, "add_bookmark", %{
      ...
      assert_broadcast_received("bookmark:firehose", "bookmark_deleted", %{
        id: bookmark_to_delete.id
      })
    end
  end
  ...
  describe "Multi-user bookmark isolation" do
    test "users only see their own bookmarks", %{user_id: user_id} do
      ...
      # Initially empty
      assert %{bookmarks: []} = reply1

      ref =
        Phoenix.ChannelTest.push(socket1, "add_bookmark", %{
          "url" => "https://user1.com",
          "title" => "User 1 Site",
          "description" => "",
          "tags" => [],
      ...
      # Should not see user 1's bookmark
  ...
  describe "Error handling and edge cases" do
    ...
    test "handles searching with empty query", %{user_id: user_id} do
      ...
      ref =
        Phoenix.ChannelTest.push(socket, "add_bookmark", %{
          "url" => "https://test.com",
          "title" => "Test",
          "description" => "",
          "tags" => [],
