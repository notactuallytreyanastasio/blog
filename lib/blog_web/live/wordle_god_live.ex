defmodule BlogWeb.WordleGodLive do
  ...
  def mount(_params, _session, socket) do
    ...
    if connected?(socket) do
      # Every minute
      :timer.send_interval(60_000, self(), :cleanup_stale_games)
    end
  ...
  def handle_event("cleanup_stale", %{"hours" => hours}, socket) do
    ...
    {:noreply,
     assign(socket, games: games, cleanup_message: "Cleaned up games older than #{hours} hour(s)")}
  end
  ...
  def handle_event("cleanup_all", _params, socket) do
    ...
    {:noreply,
     assign(socket,
       games: games,
       cleanup_message: "Removed #{before_count - after_count} invalid/problematic games"
     )}
  end
  ...
  def handle_event("cleanup_duplicates", _params, socket) do
    ...
    {:noreply,
     assign(socket,
       games: games,
       cleanup_message: "Removed #{before_count - after_count} duplicate player sessions"
     )}
  end
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <ul class="text-sm">
                ...
                <li>
                  Empty Games: <span class="font-medium text-gray-500">{@stats.empty_count}</span>
                  <span class="text-xs text-gray-500">
                    (Idle: <span class="text-gray-600 font-medium"><%= @stats.idle_empty_count %></span>)
                  </span>
                </li>
            ...
            <div>
              ...
              <ul class="text-sm">
                <li>
                  Recent (5m): <span class="font-medium text-green-600">{@stats.recent_count}</span>
                </li>
                <li>
                  Last Hour: <span class="font-medium text-yellow-600">{@stats.hour_count}</span>
                </li>
        ...
        <%= for {session_id, game} <- @games do %>
          <div class={[
            ...
            <div class="text-sm mb-2">
              ...
              <span class={[
                "ml-2",
                if(game.hard_mode, do: "text-yellow-600 font-bold", else: "text-gray-500")
              ]}>
              ...
              <%= cond do %>
                ...
                <% true -> %>
                  <span class="text-blue-600">
                    In Progress ({length(game.guesses)}/{game.max_attempts} guesses)
                  </span>
              <% end %>
            </div>
            ...
            <div class="grid grid-rows-6 gap-[3px] mb-2">
              <%= for %{word: guess, result: result} <- game.guesses do %>
                <div class="grid grid-cols-5 gap-[3px]">
                  <%= for {letter, status} <- Enum.zip(String.graphemes(guess), result) do %>
                    <div class={[
                      "w-full aspect-square flex items-center justify-center text-sm font-bold text-white rounded-none uppercase",
                      color_class(status)
                    ]}>
                      {letter}
                    </div>
              ...
              <%= if length(game.guesses) < game.max_attempts && !game.game_over do %>
                <div class="grid grid-cols-5 gap-[3px]">
                  <%= for i <- 0..4 do %>
                    <div class={[
                      "w-full aspect-square flex items-center justify-center text-sm font-bold rounded-none uppercase border",
    ...
    case DateTime.from_iso8601(timestamp) do
      {:ok, datetime, _} ->
        ...
        cond do
          ...
          seconds_diff < 300 -> "border-l-4 border-green-500"
          # Active in last hour
          seconds_diff < 3600 -> "border-l-4 border-yellow-500"
          # Active in last day
          seconds_diff < 86400 -> "border-l-4 border-orange-300"
          # Inactive for more than a day
          true -> "border-l-4 border-gray-300"
        end
  ...
  # Missing timestamp
  defp activity_class(_), do: "border-l-4 border-red-300"
  ...
  defp calculate_stats(games) do
        ...
        {active, completed, won, lost, empty} =
          cond do
            !game.game_over && length(game.guesses) == 0 ->
              {acc.active_count + 1, acc.completed_count, acc.won_count, acc.lost_count,
               acc.empty_count + 1}
            ...
            Enum.any?(game.guesses, fn %{word: word} -> word == game.target_word end) ->
              {acc.active_count, acc.completed_count + 1, acc.won_count + 1, acc.lost_count,
               acc.empty_count}

            true ->
              {acc.active_count, acc.completed_count + 1, acc.won_count, acc.lost_count + 1,
               acc.empty_count}
          end
          ...
          if length(game.guesses) == 0 do
            case DateTime.from_iso8601(game.last_activity) do
              {:ok, timestamp, _} ->
                ...
                if diff_seconds > acc.idle_threshold,
                  do: acc.idle_empty_count + 1,
                  else: acc.idle_empty_count
    ...
    %{stats | unique_players: unique_players, win_rate: win_rate, common_words: word_counts}
  end
end
