defmodule BlogWeb.JetstreamComparisonLive do
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      # Subscribe to separate feeds
      Phoenix.PubSub.subscribe(Blog.PubSub, "jetstream:skeet")
      Phoenix.PubSub.subscribe(Blog.PubSub, "relay:skeet")
    end
  ...
  # Handle messages from Jetstream topic
  def handle_info({:jetstream_skeet, %{text: text, did: did}}, socket) do
    handle_jetstream_skeet(text, did, socket)
  end

  # Handle messages from Relay topic  
  def handle_info({:relay_skeet, %{text: text, did: did}}, socket) do
    handle_relay_skeet(text, did, socket)
  end
  
  # Handle old format relay messages (fallback)
  def handle_info({:relay_skeet, text}, socket) when is_binary(text) do
    handle_relay_skeet(text, "unknown", socket)
  end

  defp handle_jetstream_skeet(skeet_text, did, socket) do
    ...
    if check_match(skeet_text, socket.assigns.search_term) do
  ...
  defp handle_relay_skeet(skeet_text, did, socket) do
      ...
      <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <input
            ...
            placeholder="Enter search term (e.g., bobby, elixir, ❤️)..."
          ...
          <div class="mt-4 grid grid-cols-2 gap-4 text-sm">
            <div class="bg-blue-50 p-3 rounded">
              ...
              <p class="text-xs mt-1">jetstream2.us-east.bsky.network</p>
            </div>
            <div class="bg-green-50 p-3 rounded">
              ...
              <p class="text-xs mt-1">bsky-relay.c.theo.io</p>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6">
          <!-- Jetstream Column -->
          <div>
            <h2 class="text-xl font-semibold mb-4 text-blue-700">Jetstream Feed</h2>
            <div class="space-y-3 max-h-screen overflow-y-auto">
              ...
              <% else %>
                <%= for skeet_item <- Enum.take(@jetstream_filtered, 50) do %>
                  ...
                  <div class="bg-white rounded-lg shadow-md p-3 border-l-4 border-blue-500">
                    <p class="text-xs text-gray-500 mb-1 font-mono truncate" title={did}>
                      {String.slice(did, 0, 30)}<%= if String.length(did) > 30 do %>...<% end %>
                    </p>
                    <p class="text-sm text-gray-800 break-words">
                      {String.slice(skeet_text, 0, 200)}<%= if String.length(skeet_text) > 200 do %>...<% end %>
                    </p>
                  </div>
                <% end %>
              <% end %>
          ...
          <!-- Relay Column -->
          <div>
            <h2 class="text-xl font-semibold mb-4 text-green-700">Relay Feed</h2>
            <div class="space-y-3 max-h-screen overflow-y-auto">
              ...
              <% else %>
                <%= for skeet_item <- Enum.take(@relay_filtered, 50) do %>
                  <% {skeet_text, did} = case skeet_item do
                    {text, did} -> {text, did}
                    text when is_binary(text) -> {text, "unknown"}
                  end %>
